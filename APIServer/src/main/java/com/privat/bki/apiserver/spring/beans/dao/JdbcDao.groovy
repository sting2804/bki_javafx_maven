...
class JdbcDao implements IBkiDao {
    ...
    private Integer addNewPerson(Person person) {
        ...
        SqlParameterSource inParam = new MapSqlParameterSource(inParamMap)

        Map<String, Object> simpleJdbcCallResult = jdbcCall.execute(inParam)
    ...
    Boolean addNewInfo(LoanInfo info, int clientId) {
        ...
        Map<String, Object> simpleJdbcCallResult = jdbcCall.execute(inParam)
        id = (int) simpleJdbcCallResult.get("ID_OUT")
        def params = [id, info.getCurrency().getCode(), info.getBank().getCode(),
